import CJS_COMPAT_NODE_URL_lpbd6hif5hp from 'node:url';
import CJS_COMPAT_NODE_PATH_lpbd6hif5hp from 'node:path';
import CJS_COMPAT_NODE_MODULE_lpbd6hif5hp from "node:module";

var __filename = CJS_COMPAT_NODE_URL_lpbd6hif5hp.fileURLToPath(import.meta.url);
var __dirname = CJS_COMPAT_NODE_PATH_lpbd6hif5hp.dirname(__filename);
var require = CJS_COMPAT_NODE_MODULE_lpbd6hif5hp.createRequire(import.meta.url);
...
import {
  require_build
} from "./chunk-L4JF77FH.js";
import {
  pLimit
} from "./chunk-6YX2ILY6.js";
import {
  isI18nPackage,
  isRouterPackage,
  isStateManagementPackage,
  isStylingPackage
} from "./chunk-5GSH4K42.js";
import {
  STORYBOOK_FN_PLACEHOLDER,
  generateDummyArgsFromArgTypes,
  loadConfig,
  printConfig
} from "./chunk-JLQ7YVOB.js";
import {
  optionalEnvToBoolean,
  registerService
} from "./chunk-N7TILUYW.js";
import {
  array,
  description,
  lazy,
  ...
  variant,
  void_
} from "./chunk-WGFB6YFF.js";
import {
  UniversalStoreFollowerTimeoutError
} from "./chunk-F53KKBXR.js";
import {
  up
} from "./chunk-LR7QCVP4.js";
import {
  resolvePackageDir
} from "./chunk-VZDXMKU7.js";
import {
  jsTsSourceExtensions
} from "./chunk-43DTN3ZZ.js";
import {
  StorybookError
} from "./chunk-4FGOCELA.js";
import {
  require_dist
} from "./chunk-AWTLUFLC.js";
import {
  require_picocolors
} from "./chunk-5WLZGZKH.js";
import {
  errorToErrorLike,
  reverseIndexToStoriesByFile,
  toStoryIndexPath
} from "./chunk-2ABO45HU.js";
import {
  extname,
  join,
  normalize,
  relative
} from "./chunk-MHFYNNS5.js";
import {
  glob
} from "./chunk-2B4WNTOI.js";
import {
  __toESM
} from "./chunk-Q4AAMLOA.js";
...
var storyIndexPathSchema = pipe(
...
), moduleGraphIndexServiceDef = defineService({
  ...
  queries: {
    _storiesForFiles: {
      internal: !0,
      description: "Internal lookup used by `core/module-graph.storiesForFiles`. Prefer the hot service query from consumers.",
      input: object({
        files: pipe(
          array(
            pipe(
  ...
  commands: {
    _applyIndex: {
      internal: !0,
      description: "Replaces the reverse index. Called by `core/module-graph` snapshot/update commands when the index moved.",
      input: object({
        storiesByFile: pipe(
          storiesByFileSchema,
          description(
...
var errorLikeSchema = object({
...
]), storyIndexPathSchema2 = pipe(
  string(),
  description("A story-index-style relative path such as `./src/Button.stories.tsx`.")
), storyDependencyDepthSchema2 = pipe(
  number(),
  description(
    "Breadth-first-search depth: the shortest number of import edges between the source file and this story file."
  )
), storiesByFileSchema2 = record(
  storyIndexPathSchema2,
  record(storyIndexPathSchema2, storyDependencyDepthSchema2)
      ...
      output: array(
        array(
          object({
            ...
            depth: storyDependencyDepthSchema2
          })
        )
      ),
      handler: (input, ctx) => ctx.getService("core/module-graph-index").queries._storiesForFiles.get(input)
    },
    status: {
  ...
  commands: {
    _applyGraphSnapshot: {
      ...
      output: void_(),
      handler: async (input, ctx) => {
        await ctx.getService("core/module-graph-index").commands._applyIndex({
    ...
    _applyGraphUpdate: {
      internal: !0,
      description: "Optionally replaces the reverse index after an incremental patch and bumps versions for affected story files. Called by the graph engine, not by external consumers.",
      input: object({
        storiesByFile: optional(
          pipe(
            storiesByFileSchema2,
            description(
      ...
      output: void_(),
      handler: async (input, ctx) => {
        input.storiesByFile !== void 0 && await ctx.getService("core/module-graph-index").commands._applyIndex({
          storiesByFile: input.storiesByFile
        }), input.bumpedStoryFiles.length !== 0 && ctx.self.setState((state) => {
...
var ModuleGraphEngine = class {
  ...
  async mirrorUpdate(changedFile, prePatchBumped, indexChanged) {
    ...
    indexChanged && await this.options.onIndex?.(
      reverseIndexToStoriesByFile(this.reverseIndex.asMap(), this.workingDir)
    ), bumpedStoryFiles.size > 0 && await this.options.onBump?.(
...
function registerModuleGraphService(options) {
  let workingDir = options.workingDir ?? process.cwd(), engine, indexRuntime = registerModuleGraphIndexService(workingDir), runtime = registerService(
    {
...
var getComponentVariableName = async (name) => (await import("./camelcase-U3VGLPYQ.js")).default(name.replace(/^[^a-zA-Z_$]*/, ""), { pascalCase: !0 }).replace(/[^a-zA-Z_$]+/, "");
